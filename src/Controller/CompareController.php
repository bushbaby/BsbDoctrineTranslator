    ...
    public function compareSourceToDatabaseAction()
    {
        ...
        if (count($moreThenOneEntities)) {
            $select = Select::prompt(
                sprintf(
                    "Detected %s duplicate entities(s) within the database. What do you want to do?\n",
        ...
        if (count($sourcesNotInDatabase)) {
            $select = Select::prompt(
                sprintf(
                    "Detected %s suitable message(s) within your source do not have a entry in the database. What do you want to do?\n",
