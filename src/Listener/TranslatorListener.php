...
class TranslatorListener implements ListenerAggregateInterface
{
    ...
    {
//        $this->listeners[] =
//          $events->attach(Translator::EVENT_MISSING_TRANSLATION, array($this, 'translationMissing'));
//        $this->listeners[] =
//          $events->getSharedManager()->attach('*', MvcEvent::EVENT_FINISH, array($this, 'finished'), 10000);
    }
    ...
    public function finished(MvcEvent $event)
    {
        ...
        $repo1 = $this->entityManager->getRepository('BsbDoctrineTranslatorLoader:Locale');
        $repo2 = $this->entityManager->getRepository('BsbDoctrineTranslatorLoader:Message');
        ...
        foreach ($this->entries as $message) {
            ...
            foreach ($Locales as $Locale) {
                $Message = $repo2->findOneBy(array('locale'  => $Locale,
                                                   'message' => $message['message'],
                                                   'domain'  => $message['text_domain']
                    ));
                if (!$Message) {
                    $Message = new Message();
